...
const convertUploadImageToWebp = async (file) => {
  ...
  const quality = Math.min(
    100,
    Math.max(1, Number(process.env.WEBP_QUALITY || 72)),
  );
  ...
  try {
    ...
    console.warn("Image conversion to WebP failed, using original file:", error);
    return toRelativeUploadPath(file.path);
  }
};
...
export const addBlog = async (req, res) => {
  try {
    ...
    if (req.body.blog) {
      ({ title, subTitle, description, category, isPublished } = JSON.parse(
        req.body.blog,
      ));
    } else {
    ...
    const imagePath = imageFile
      ? await convertUploadImageToWebp(imageFile)
      : null;
    ...
    res
      ...
      .json({
        success: false,
        message: "We couldn't publish this post right now. Please try again.",
      });
  }
};
...
export const getAllBlogs = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "We could not load posts right now. Please try again.",
      });
  }
};
...
export const getBlogById = async (req, res) => {
  try {
    ...
    if (!blog) {
      return res
        .status(404)
        .json({ success: false, message: "This post could not be found." });
    }
    ...
    res
      ...
      .json({
        success: false,
        message: "We could not load this post right now. Please try again.",
      });
  }
};
...
export const deleteBlog = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "We could not delete this post right now. Please try again.",
      });
  }
};
  ...
  try {
    ...
    res
      ...
      .json({
        success: false,
        message:
          "We could not update publish status right now. Please try again.",
      });
  }
};
...
export const updateBlogCategory = async (req, res) => {
  try {
    ...
    if (!id || !nextCategory) {
      return res
        .status(400)
        .json({ success: false, message: "Blog id and category are required" });
    }
...
export const updateBlog = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "We could not update this post right now. Please try again.",
      });
  }
};
...
export const addComment = async (req, res) => {
  try {
    ...
    if (!blogId || !name || !comment) {
      return res
        .status(400)
        .json({ message: "Blog ID, name, and comment are required." });
    }
    ...
    res
      ...
      .json({
        success: false,
        message: "We could not add your comment right now. Please try again.",
      });
  }
};
...
export const getComments = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "We could not load comments right now. Please try again.",
      });
  }
};
  ...
  try {
    ...
    res
      ...
      .json({
        success: false,
        message:
          "Content generation is unavailable right now. Please try again.",
      });
  }
};
